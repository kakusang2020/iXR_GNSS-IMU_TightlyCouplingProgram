...
heightlist=[0,-20];time_last_GNSS = round(stime,1);speedvech=0;heading=att(3);speedtest=[];headingCheckList=[];debug=[];a=0;gyrobias=[];oldgyrobias_=0;TestList=[];%speedhor_=27.2315;
biasupdatetime=0;
...
for k =find(round(IMUData(:,1),2)==round(stime,2)):find(round(IMUData(:,1),2)==round(etime,2))
    ...
    %         gyrobias=[];
    %%     end
    u_h = [IMUData(k,2:6)' - delta_u(1:5);IMUData(k,7)-oldgyrobias_];
    % 捷联惯导解算
    ...
    %     % 垂直方向速度
    heightincrement=44300*(1- (carSpeed(8)/1015.25)^(1/5.256)) - 44300*(1- (carSpeedold(8)/1015.25)^(1/5.256));
    heightlist=[heightlist;IMUData(k,1),heightlist(end,2)+heightincrement]; %heightincrement list
    if length(heightlist)>100 && dt~=0
        speedvech=(median(heightlist(end-20:end))-median(heightlist(end-100:end-80)))/dt/100;
    end
    speedvech=0;%debug
    if carSpeed(5)<=abs(speedvech*5)
        speedvech=0;
    end
